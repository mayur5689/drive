...
export default function FilesPage() {
    ...
    const [dbEnrichment, setDbEnrichment] = useState<DBEnrichment>({ clients: [], requests: [] });
    ...
    useEffect(() => {
        const fetchInitial = async () => {
            setIsLoading(true);
            try {
                // 1. Get root folder ID from settings
                const settingsRes = await fetch('/api/settings');
                const settings = await settingsRes.json();
                const rid = settings.drive_root_folder_id;

                if (rid) {
                    setRootFolderId(rid);
                    setCurrentDriveFolderId(rid);
                    setDriveBreadcrumbs([{ id: rid, name: 'Files' }]);
                    await browseDriveFolder(rid);
                }

                // 2. Fetch Clients & Requests for enrichment
                const [clientsRes, requestsRes] = await Promise.all([
                    fetch('/api/clients'),
                    fetch('/api/requests')
                ]);

                if (clientsRes.ok && requestsRes.ok) {
                    const clients = await clientsRes.json();
                    const requests = await requestsRes.json();
                    setDbEnrichment({
                        clients: clients.map((c: any) => ({ id: c.id, name: c.name, org: c.organization })),
                        requests: requests.map((r: any) => ({ id: r.id, title: r.title }))
                    });
                }
            } catch (error) {
